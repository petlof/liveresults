...
module LiveResults.Competition {
    ...
    export class CompetitionController {
        ...
        static $inject = ["$routeParams", "$scope", "$http", "apiUrl"];
        constructor(private $routeParams: any,
            private $scope: ICompetitionScope, private $http: ng.IHttpService, private apiUrl : string) {
            this.competitionId = $routeParams["competition"];
            this.updateClasses();
            $http.get(this.apiUrl + '?comp=10640&method=getclassresults&unformattedTimes=true&class=W21&last_hash=06d8bafbe4e8edf6ef825d944828d93e').success((data : any) => {
