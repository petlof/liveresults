...
classUpdateTimer=setTimeout(LiveResults.Instance.updateClassList,classUpdateInterval);};this.setShowTenth=function(val){_showThenthOfSecond=val;}
...
{return formatTime(o.aData.result,o.aData.status,false,true,_showThenthOfSecond)+" ("+o.aData.place+")";}}});col++;columns.push({"sTitle":"Status","bVisible":false,"aTargets":[col++],"sType":"numeric","mDataProp":"status"});columns.push({"sTitle":"","sClass":"center","bSortable":false,"aTargets":[col++],"mDataProp":"timeplus","fnRender":function(o,val)
{if(o.aData.status!=0)
return"";else
return"+"+formatTime(o.aData.timeplus,o.aData.status,false,true,_showThenthOfSecond);}});if(_isMultiDay)
...
{if(val.sTitle=="VP")
{idxCol=idx;}});currentTable.fnSort([[idxCol,'asc']]);$("#"+_txtResetSorting).html("");};formatTime=function(time,status,showHours,padZeros,showTenthOS)
...
{if(showHours)
{hours=Math.floor(time/360000);minutes=Math.floor((time-hours*360000)/6000);seconds=Math.floor((time-minutes*6000-hours*360000)/100);var tenth=Math.floor((time-minutes*6000-hours*360000-seconds*100)/10);if(hours>0)
{if(padZeros)
...
{minutes=Math.floor(time/6000);seconds=Math.floor((time-minutes*6000)/100);var tenth=Math.floor((time-minutes*6000-seconds*100)/10);if(padZeros){return str_pad(minutes,2)+":"+str_pad(seconds,2)+(showTenthOS?"."+tenth:"");}
