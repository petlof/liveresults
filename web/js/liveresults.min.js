...
{$.ajax({url:"api.php",data:"comp="+_compId+"&method=getclassresults&unformattedTimes=true&class="+curClassName+"&last_hash="+lastClassHash+(_isMultiDay?"&includetotal=true":""),success:resp_updateClassResults,error:function(xhr,ajaxOptions,thrownError){resUpdateTimeout=setTimeout(checkForClassUpdate,updateInterval);},dataType:"json"});}}};resp_updateClassResults=function(data)
{if(data.status=="OK")
{if(currentTable!=null)
...
resUpdateTimeout=setTimeout(checkForClassUpdate,updateInterval);};this.updateClassResults=function(data)
{if(data!=null&&data.status=="OK")
{if(data.className!=null)
{$('#'+_resultsHeaderDiv).html('<b>'+data.className+'</b>');$('#'+_resultsControlsDiv).show();}
if(data.results!=null)
{columns=Array();columns.push({"sTitle":"#","bSortable":false,"aTargets":[0],"mDataProp":"place"});columns.push({"sTitle":Resources["_NAME"],"bSortable":false,"aTargets":[1],"mDataProp":"name"});columns.push({"sTitle":Resources["_CLUB"],"bSortable":false,"aTargets":[2],"mDataProp":"club"});curClassSplits=data.splitcontrols;updateResultVirtualPosition(data.results);var col=3;columns.push({"sTitle":Resources["_START"],"sClass":"left","sType":"numeric","aDataSort":[col],"aTargets":[col],"bUseRendered":false,"mDataProp":"start","fnRender":function(o,val)
{if(o.aData.start=="")
{return"";}
else
...
{window.open('followfull.php?comp='+_compId+'&lang='+_lang+'&class='+curClassName,'','status=0,toolbar=0,location=0,menubar=0,directories=0,scrollbars=1,resizable=1,width=900,height=600');};this.chooseClass=function(className)
{if(currentTable!=null)
currentTable.fnDestroy();clearTimeout(resUpdateTimeout);$('#divResults').html('');curClassName=className;$('#resultsHeader').html(Resources["_LOADINGRESULTS"]);$.ajax({url:"api.php",data:"comp="+compId+"&method=getclassresults&unformattedTimes=true&class="+className+(_isMultiDay?"&includetotal=true":""),success:this.updateClassResults,dataType:"json"});if(!_isSingleClass){window.location.hash=className;}
