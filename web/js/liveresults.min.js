...
{$.ajax({url:"api.php",data:"comp="+_compId+"&method=getclassresults&unformattedTimes=true&class="+curClassName+"&last_hash="+lastClassHash+(_isMultiDay?"&includetotal=true":""),success:resp_updateClassResults,error:function(xhr,ajaxOptions,thrownError){resUpdateTimeout=setTimeout(checkForClassUpdate,updateInterval);},dataType:"json"});}}};checkForClubUpdate=function()
{if(updateAutomatically)
{if(currentTable!=null)
{$.ajax({url:"api.php",data:"comp="+_compId+"&method=getclubresults&unformattedTimes=true&club="+encodeURIComponent(curClubName)+"&last_hash="+lastClubHash+(_isMultiDay?"&includetotal=true":""),success:resp_updateClubResults,error:function(xhr,ajaxOptions,thrownError){resUpdateTimeout=setTimeout(checkForClubUpdate,updateInterval);},dataType:"json"});}}};resp_updateClassResults=function(data)
{if(data.status=="OK")
{if(currentTable!=null)
...
if(data.results!=null)
{columns=Array();columns.push({"sTitle":"#","aTargets":[0],"mDataProp":"place"});columns.push({"sTitle":Resources["_NAME"],"aTargets":[1],"mDataProp":"name"});columns.push({"sTitle":Resources["_CLUB"],"bSortable":false,"aTargets":[2],"mDataProp":"club"});var col=3;columns.push({"sTitle":Resources["_START"],"sClass":"left","sType":"numeric","aDataSort":[col],"aTargets":[col],"bUseRendered":false,"mDataProp":"start","fnRender":function(o,val)
{if(o.aData.start=="")
{return"";}
else
...
{if(currentTable!=null)
currentTable.fnDestroy();clearTimeout(resUpdateTimeout);$('#divResults').html('');curClubName=clubName;curClassName=null;$('#resultsHeader').html(Resources["_LOADINGRESULTS"]);$.ajax({url:"api.php",data:"comp="+compId+"&method=getclubresults&unformattedTimes=true&club="+encodeURIComponent(clubName)+(_isMultiDay?"&includetotal=true":""),success:this.updateClubResults,dataType:"json"});if(!_isSingleClass){window.location.hash="club::"+clubName;}
...
$('#resultsHeader').html(Resources["_LOADINGRESULTS"]);$.ajax({url:"api.php",data:"comp="+compId+"&method=getclassresults&unformattedTimes=true&class="+className+(_isMultiDay?"&includetotal=true":""),success:this.updateClassResults,dataType:"json"});if(!_isSingleClass){window.location.hash=className;}
