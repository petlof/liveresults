...
classUpdateTimer=setTimeout(LiveResults.Instance.updateClassList,classUpdateInterval);};this.setShowTenth=function(val){_showThenthOfSecond=val;};this.updateLastPassings=function()
{if(updateAutomatically)
...
{$.ajax({url:"api.php",data:"comp="+_compId+"&method=getclassresults&unformattedTimes=true&class="+encodeURIComponent(curClassName)+"&last_hash="+lastClassHash+(_isMultiDay?"&includetotal=true":""),success:resp_updateClassResults,error:function(xhr,ajaxOptions,thrownError){resUpdateTimeout=setTimeout(checkForClassUpdate,updateInterval);},dataType:"json"});if(typeof(_gaq)=="object")
{_gaq.push(['_trackPageview','/'+_compId+'/'+curClassName]);}}}};checkForClubUpdate=function()
{if(updateAutomatically)
{if(currentTable!=null)
{$.ajax({url:"api.php",data:"comp="+_compId+"&method=getclubresults&unformattedTimes=true&club="+encodeURIComponent(curClubName)+"&last_hash="+lastClubHash+(_isMultiDay?"&includetotal=true":""),success:resp_updateClubResults,error:function(){resUpdateTimeout=setTimeout(checkForClubUpdate,updateInterval);},dataType:"json"});if(typeof(_gaq)=="object")
...
{var columns=Array();columns.push({"sTitle":"#","bSortable":false,"aTargets":[0],"mDataProp":"place"});columns.push({"sTitle":Resources["_NAME"],"bSortable":false,"aTargets":[1],"mDataProp":"name"});columns.push({"sTitle":Resources["_CLUB"],"bSortable":false,"aTargets":[2],"mDataProp":"club","fnRender":function(o)
{return"<a href=\"javascript:LiveResults.Instance.viewClubResults('"+o.aData.club+"')\">"+o.aData.club+"</a>";}});curClassSplits=data.splitcontrols;updateResultVirtualPosition(data.results);var col=3;columns.push({"sTitle":Resources["_START"],"sClass":"left","sType":"numeric","aDataSort":[col],"aTargets":[col],"bUseRendered":false,"mDataProp":"start","fnRender":function(o)
{if(o.aData.start=="")
{return"";}
else
{return formatTime(o.aData.start,false,0,true);}}});col++;if(data.splitcontrols!=null)
...
return"";else
return formatTime(o.aData.splits[value.code],0)+" ("+o.aData.splits[value.code+"-place"]+")";}});col++;columns.push({"sTitle":value.name+"_Status","bVisible":false,"aTargets":[col++],"sType":"numeric","mDataProp":"splits."+value.code+"_status"});});}
var timecol=col;columns.push({"sTitle":Resources["_CONTROLFINISH"],"sClass":"left","sType":"numeric","aDataSort":[col+1,col,0],"aTargets":[col],"bUseRendered":false,"mDataProp":"result","fnRender":function(o)
{if(o.aData.place=="-"||o.aData.place=="")
{return formatTime(o.aData.result,o.aData.status,_showThenthOfSecond);}
else
{return formatTime(o.aData.result,o.aData.status,_showThenthOfSecond)+" ("+o.aData.place+")";}}});col++;columns.push({"sTitle":"Status","bVisible":false,"aTargets":[col++],"sType":"numeric","mDataProp":"status"});columns.push({"sTitle":"","sClass":"center","bSortable":false,"aTargets":[col++],"mDataProp":"timeplus","fnRender":function(o)
{if(o.aData.status!=0)
return"";else
return"+"+formatTime(o.aData.timeplus,o.aData.status,_showThenthOfSecond);}});if(_isMultiDay)
{columns.push({"sTitle":Resources["_TOTAL"],"sClass":"left","sType":"numeric","aDataSort":[col+1,col,0],"aTargets":[col],"bUseRendered":false,"mDataProp":"totalresult","fnRender":function(o)
{if(o.aData.totalplace=="-"||o.aData.totalplace=="")
{return formatTime(o.aData.totalresult,o.aData.totalstatus);}
else
{return formatTime(o.aData.totalresult,o.aData.totalstatus)+" ("+o.aData.totalplace+")";}}});col++;columns.push({"sTitle":"TotalStatus","bVisible":false,"aTargets":[col++],"sType":"numeric","mDataProp":"totalstatus"});columns.push({"sTitle":"","sClass":"center","bSortable":false,"aTargets":[col++],"mDataProp":"totalplus","fnRender":function(o)
...
if(data.results!=null)
{var columns=Array();columns.push({"sTitle":"#","aTargets":[0],"mDataProp":"place"});columns.push({"sTitle":Resources["_NAME"],"aTargets":[1],"mDataProp":"name"});columns.push({"sTitle":Resources["_CLUB"],"bSortable":false,"aTargets":[2],"mDataProp":"club"});columns.push({"sTitle":Resources["_CLASS"],"aTargets":[3],"mDataProp":"class","fnRender":function(o)
{return"<a href=\"javascript:LiveResults.Instance.chooseClass('"+o.aData["class"]+"')\">"+o.aData["class"]+"</a>";}});var col=4;columns.push({"sTitle":Resources["_START"],"sClass":"left","sType":"numeric","aDataSort":[col],"aTargets":[col],"bUseRendered":false,"mDataProp":"start","fnRender":function(o)
{if(o.aData.start=="")
{return"";}
else
{return formatTime(o.aData.start,0,false,true);}}});col++;var timecol=col;columns.push({"sTitle":Resources["_CONTROLFINISH"],"sClass":"left","sType":"numeric","aDataSort":[col+1,col,0],"aTargets":[col],"bUseRendered":false,"mDataProp":"result","fnRender":function(o)
{if(o.aData.place=="-"||o.aData.place=="")
{return formatTime(o.aData.result,o.aData.status);}
else
{return formatTime(o.aData.result,o.aData.status)+" ("+o.aData.place+")";}}});col++;columns.push({"sTitle":"Status","bVisible":false,"aTargets":[col++],"sType":"numeric","mDataProp":"status"});columns.push({"sTitle":"","sClass":"center","bSortable":false,"aTargets":[col++],"mDataProp":"timeplus","fnRender":function(o)
{if(o.aData.status!=0)
return"";else
...
{var idxCol=0;$.each(currentTable.fnSettings().aoColumns,function(idx,val)
{if(val.sTitle=="VP")
{idxCol=idx;}});currentTable.fnSort([[idxCol,'asc']]);$("#"+_txtResetSorting).html("");};formatTime=function(time,status,showTenthOs,showHours,padZeros)
...
else{var minutes;var seconds;var tenth;if(showHours)
{var hours=Math.floor(time/360000);minutes=Math.floor((time-hours*360000)/6000);seconds=Math.floor((time-minutes*6000-hours*360000)/100);tenth=Math.floor((time-minutes*6000-hours*360000-seconds*100)/10);if(hours>0)
{if(padZeros)
hours=str_pad(hours,2);return hours+":"+str_pad(minutes,2)+":"+str_pad(seconds,2)+(showTenthOs?"."+tenth:"");}
else
{if(padZeros)
minutes=str_pad(minutes,2);return minutes+":"+str_pad(seconds,2)+(showTenthOs?"."+tenth:"");}}
else
{minutes=Math.floor(time/6000);seconds=Math.floor((time-minutes*6000)/100);tenth=Math.floor((time-minutes*6000-seconds*100)/10);if(padZeros){return str_pad(minutes,2)+":"+str_pad(seconds,2)+(showTenthOs?"."+tenth:"");}
else{return minutes+":"+str_pad(seconds,2)+(showTenthOs?"."+tenth:"");}}}};str_pad=function(number,length){var str=''+number;while(str.length<length){str='0'+str;}
...
{if(currentTable!=null)
currentTable.fnDestroy();clearTimeout(resUpdateTimeout);$('#divResults').html('');curClassName=className;curClubName=null;$('#resultsHeader').html(Resources["_LOADINGRESULTS"]);$.ajax({url:"api.php",data:"comp="+compId+"&method=getclassresults&unformattedTimes=true&class="+encodeURIComponent(className)+(_isMultiDay?"&includetotal=true":""),success:this.updateClassResults,dataType:"json"});if(typeof(_gaq)=="object")
...
{var i;if(curClassSplits!=null)
{for(i=0;i<data.length;i++)
{data[i].haveSplits=false;}
for(var s=0;s<curClassSplits.length;s++)
{var splitCode=curClassSplits[s].code;data.sort(function(a,b){return a.splits[splitCode]-b.splits[splitCode];});var lastPos=1;var posCnt=1;var lastTime=-1;for(i=0;i<data.length;i++)
...
data.sort(resultSorter);var firstFinishedIdx=-1;for(i=0;i<data.length;i++)
{if(data[i].place!="")
{firstFinishedIdx=i;break;}}
if(firstFinishedIdx==-1)
firstFinishedIdx=data.length-1;var tmp=Array();for(i=0;i<firstFinishedIdx;i++)
{tmp.push(data[i]);}
...
{for(s=curClassSplits.length-1;s>=0;s--){var splitCode=curClassSplits[s].code;if(a.splits[splitCode]==""&&b.splits[splitCode]!="")
return 1;else if(a.splits[splitCode]!=""&&b.splits[splitCode]=="")
return-1;}
return 0;};insertIntoResults=function(result,data)
{var haveSplit=false;var d;if(curClassSplits!=null)
{for(s=curClassSplits.length-1;s>=0;s--){var splitCode=curClassSplits[s].code;if(result.splits[splitCode]!="")
...
{data.splice(d,0,result);return;}}}
data.push(result);};resultSorter=function(a,b)
{if(a.place!=""&&b.place!="")
...
{return 0;}};$(window).hashchange(function(){if(window.location.hash){var hash=window.location.hash.substring(1);var cl;if(hash.indexOf('club::')>=0)
{cl=hash.substring(6);if(cl!=curClubName)
{LiveResults.Instance.viewClubResults(hash.substring(6));}}
else
{cl=hash;if(cl!=curClassName)
