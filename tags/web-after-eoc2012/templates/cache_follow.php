...
<?if ($updTime != "-1") {?> <META HTTP-EQUIV="refresh" CONTENT="<?=$updTime?>;"><?}?>
...
<!-- MAIN MENU FLAPS - Two rows, note that left and right styles differs from middle ones -->
  ...
  <tr>
    <td class="submenu" colspan="2">
       <table border="0" cellpadding="0" cellspacing="0" width="100%">
             <tr>
               <td><a href="../index.php?lang=<?=$lang?>&"><?=$_CHOOSECMP?></a> >> <?=$currentComp->CompName()?> [<?=$currentComp->CompDate()?>] <?= ($_GET['class'] != '' ? ">> ".$_GET['class'] : "") ?></td>
...
<?
			...
			foreach ($lastPassings as $pass)
			  {
			    echo(date("H:i:s",strtotime($pass['Changed'])).": ".$pass['Name']." (".$pass['class'].") ". (($pass['Control'] == "1000" || $pass['Control'] == "1999") ? $_LASTPASSFINISHED : $_LASTPASSPASSED." ".$pass['pname'])." $_LASTPASSWITHTIME " .formatTime($pass['Time'],$pass['Status'],$RunnerStatus)." <br>");
			   			  }
?>
</td></tr></table><br>
			<table border="0" cellpadding="0" cellspacing="0" width="100%">
...
<?
	...
	else
	{
		...
		{
			if ($_GET['comp'] == 1051 && substr($cl['Class'],-1) == 'A')
				continue;
		...
		{
			echo("<a href='".$lang."_". $_GET['comp'] ."_".urlencode($cl['Class'])."_1000.html'>".$cl['Class']."</a><br>");
		}
		}
	}
?>
...
<?
	...
	foreach($splitControls as $val)
	  {
		...
		echo("<b>". $val['name'] . "</b> | ");
	      ...
	      {
	    echo("<a href='".$lang."_". $_GET['comp'] ."_". urlencode($_GET['class']). "_".$val['code'].".html'>".$val['name']."</a> | ");
	      }
	  }
	if (!isset($_GET['split']) || $_GET['split'] == "1000")
	  ...
	  {
	    echo("<a href='".$lang."_". $_GET['comp'] ."_". urlencode($_GET['class']). "_1000.html'>$_CONTROLFINISH</a> |");
	  }
?>
<table border="0" cellpadding="0" cellspacing="0" width="100%">
<tr>
...
if ($currentClass != '')
{
  if (isset($_GET['split']) && $_GET['split'] != 1000)
    ...
    {
	///Temp fix!! For SI-Timec shit!
      $results = $currentComp->getSplitsForClass($currentClass,1999);
    }
$evenColor = "#DDDDDD";
$newColor = "#FFDDDD";
 $place = "";
...
<p align=center>&copy;2006-2008, <?=$_NOTICE?>, <br/><font color=gray>Page generated: <?= date("Y-m-d H:i:s")?></font></p>
...
else
{
 ...
 if(isset($_GET['split']))
  ...
  $fp = @fopen("../cache/".$lang."_".$_GET['comp']."_".urlencode($_GET['class'])."_$split.html", 'w');
